...
public class EstudianteServiceImpl implements IEstudianteService { // Define la clase EstudianteServiceImpl que implementa la interfaz IEstudianteService
    ...
    public EstudianteDTO obtenerEstudiantePorId(Long id) { // Método para obtener un estudiante por su ID
        Estudiante estudiante = estudianteRepository.findById(id).orElseThrow(() -> new RuntimeException("Estudiante no encontrado")) ; // Busca el estudiante por ID en el repositorio y si no se encuentra lanza una excepción
    ...
    public EstudianteDTO actualizarEstudiante(Long id, EstudianteDTO estudianteDTO) { // Método para actualizar un estudiante
        Estudiante estudianteExistente = estudianteRepository.findById(id).orElseThrow(() -> new RuntimeException("Estudiante no encontrado")); // Busca el estudiante por ID en el repositorio y si no se encuentra lanza una excepción
        estudianteExistente.setNombre(estudianteDTO.getNombre()); // Actualiza el nombre del estudiante existente
        estudianteExistente.setApellido(estudianteDTO.getApellido()); // Actualiza el apellido del estudiante existente
        estudianteExistente.setEmail(estudianteDTO.getEmail()); // Actualiza el email del estudiante existente
        estudianteExistente.setFechaNacimiento(estudianteDTO.getFechaNacimiento()); // Actualiza la fecha de nacimiento del estudiante existente
        estudianteExistente.setNumeroInscripcion(estudianteDTO.getNumeroInscripcion()); // Actualiza el número de inscripción del estudiante existente
        Estudiante estudianteActualizado = estudianteRepository.save(estudianteExistente); // Guarda el estudiante actualizado en el repositorio
    ...
    public EstudianteDTO crearEstudiante(EstudianteDTO estudianteDTO) { // Método para crear un nuevo estudiante
        ...
        Estudiante estudianteGuardado = estudianteRepository.save(estudiante); // Guarda el nuevo estudiante en el repositorio
        return convertToDTO(estudianteGuardado); // Convierte el estudiante creado a EstudianteDTO y lo retorna
    }
    @Override // Anotación que indica que este método sobrescribe un método de la interfaz
    public void eliminarEstudiante(Long id) { // Método para eliminar un estudiante por su ID
        if (!estudianteRepository.existsById(id)) { // Verifica si el estudiante existe en el repositorio
            throw new RuntimeException("Estudiante no encontrado con ID: "+id); // Si no existe, lanza una excepción
        }
        estudianteRepository.deleteById(id); // Elimina el estudiante por ID en el repositorio
    }
